...
Internet end user devices\footnote{We define end user devices as personal computers or consumer devices like PDA's or smart phones.} are usually members in private local area networks, hidden to the internet behind routers and firewalls.
It is not possible for such a device to directly address another peer if they do not share the same local network.
Rather, local networks might be addressable by only one public ip address and the router must translate the packets to the correct peers (NAT).
...
In this rather volatile environment some sort of external service is needed to establish connections interactively\footnote{Interactive Connectivity Establishment (ICE)} to traverse network borders.
NAT traversal and the need for some sort of signalling service that acquaints peers with each others makes establishing connections between peers more complex, more fragile and slower than it is in local networks.
...
Many gossip protocols build up upon the premise that the peers can interconnect easily with each other and, as such, constantly connecting to a new set of peers is the basis of many algorithms\cite{Voulgaris05cyclon:inexpensive}\cite{Jelasity:2004:PSS:1045658.1045666}.
Unfortunally, in the context of the internet, we need some sort of global signalling service to introduce peers to each other prior to connection - we will call this process \emph{handshaking}.
This potentially makes the system less scalable as a gossip protocol usually promisses.

% 3:summarize main contributions. What is the general approach taken? Why significant? MUST BE GOOD!

In this report, we show how to integrate handshake 
