...
Assuming a high number of $n$ processes that work on a moderate to high number of $m$  documents and that all processes are connected to one network to enjoy the advantages of a gossip protocol (resilent, healing abilitiy).

Assuming processes $p_1, ..., p_\frac{n}{m}$ work together on document $d_1$ under a fully randomized gossip protocol.

If $p_1$ wants to broadcast a message to all processes related to document $d_1$ we expect $(n-1)H_\frac{n}{m}$ messages to be gossiped until $p_2$ to $p_\frac{n}{m}$ are notfied (\emph{\color{red} is that assumption right or is it BS??}).

Assuming that all processes are connected to one peer-to-peer network and that a very high number of $\mathit{n}$  processes will work on a moderate to high number of $\mathit{m}$ Documents ($n \geq m$) we will run into  
\section{Objectives}
