...
\title{Evaluating Gossip Protocols in Wide Area Networks}
...
Many algorithms work well in local area networks where addressing other peers is simple and relativly fast (\footnote{\color{red} I probably do need to reference this 'asumption'?}).
In practice, however, peers are often members in extended networks (WAN) where they are scattered over multiple local networks - one example would be the internet. 
...
The contribution of this report is the examination of various important gossip protocols run in a wide area network.
