...
\title{A Scalable Network Topology for Massive Collaborative Editing}
...
Distributed collaborative editors like Google Docs, Etherpad or Microsoft Word Online enable users to work together on documents not being hindered by space, time or organization. Every user of an collaborative system has the opportunity to write, edit or remove text, changes made to the document will reflect almost immediately to all other users who work on the same document.
Users can also work on multiple documents at the same time, interacting with different groups of people.

Internally every user holds a local copy of the document and broadcasts changes made to it while integrating changes made from remote users. Almost all current solutions utilize a Client-Server-Infrastructure with the Server being the final decision instance in case of a conflict.

The centralization presents certain problems: The Server is a Single Point of Failure and introduces privacy and economic intelligence issues as well as limitations in terms of service and scalability. A possible solution to this problems is the full decentralization of the system - local replicas can be synchronized through a Conflict-free Replicated Data Type (CRDT - Commutative Replicated Data Type) which provide an eventual consistency without the need of an authority Server. Users are not connected in a star-network as imposed by the Server-Client-Architecture but are connected through a peer-to-peer based gossip protocol. We will call a user connected to a gossip protocol and working on one or more documents a process.

A gossip protocol has the advantage of being highly resilent and scalable at the same time. A message is broadcast to $\mathit{c}$ (also called FANOUT) randomly selected processes from the network. Likewise, the receiving processes broadcast the message to $\mathit{c}$ randomly selected processes and so forth.
Message buffers and causality tracking ensure synchronicity between all peers.

\section{Statement of the problem}

Assuming a high number of $n$ processes that work on a moderate to high number of $m$  documents - all processes are connected to one ranomized gossip network to enjoy the advantages of a gossip protocol (resilent, healing ability).
Suppose processes $p_1, ..., p_\frac{n}{m}$ work together on document $d_1$:

If $p_1$ wants to broadcast a message to all processes related to document $d_1$ we expect $(n-1)\pmb{H}_\frac{n}{m}$ messages to be gossiped until $p_2$ to $p_\frac{n}{m}$ are notfied (\emph{\color{red} is that assumption right or is it BS??}) - in a big network this will result in a high delay of document synchronisation between processes, furthermore it will generate a vast amount of unnecessary message sending as most of the processes are not interested in changes made to documents they are not working on.

\section{Objectives}

Objective of this paper is to present an algorithm to construct a network topology that incorporates the logical structure between processes that is laid out by documents and, furthermore, ensure that messages create a minimum of unnecessary noise - this topology will be called \textit{target topology}.


\section{Work achieved}

Achived goals of this work are the prototypal implementation of the T-Man-Algorithm for a Website using WebRTC as peer-to-peer technology.
...
\begin{itemize}
    \item{\textbf{Chapter~\ref{chap:StateOfTheArt}} studies related proposals of the scientific literature addressing topics such as gossip and membership protocols.}
    \item{\textbf{Chapter~\ref{chap:Proposals}} describes the profile and ranking function for the T-Man Algorithm to build the \textit{target topology}.}
...
\textit{Gossip-based broadcast algorithms, a familiy of probabilistic broadcast algorithms, trade reliability guarantees against "scalability" properties}\footnote{Lightwight Probabilistic Broadcast - P. TH. EUGSTER, R. GUERRAOUI, S.B. HANDURUKANDE and P. KOUZNETSOV}.

As the name suggests, gossip, or epidemic, algorithms are asynchronous protocols that build upon how rumors, or diseases, spread in a population.

Consider a network of $n$ nodes denoted by $V=\{1,...,n\}$. Let $E \subset V \times V$ denote the bidirectional links along which node pairs can communicate, let this network graph be denoted by $G = (V,E)$ where $G$ constantly changes in terms of $V$ and $E$ and let $d_i$ denote the degree of node $i$ in $G$.
\footnote{Gossip Algorithms - Devavrat SHAH - \emph{\color{red}Is this a correct citation??? IMPORTANT}}

If a node $i$ wants to broadcast a message it selects $c$ random nodes from $V\setminus\{i\}$ to which it will send the message - receiving nodes will send it to another $c$ randomly selected nodes and so forth.
On receiving a message a second time a node discards a message - the problem of detecting this is out of the scope of this report, it has been addressed in \emph{\color{red}Find a relevant paper!!}. 

For configurating the protocol there are two important parameters:
\begin{itemize}
    \item \textbf{Fanout:} denoted as $c$ in the description above. Defines the number of nodes that a message is broadcasted to  by one node. A high fanout value results in higher redundant network traffic but also guarantees a higher fault tolerance.
    \item \textbf{TTL:} Number of hopes a message will take until it will be discared.
\end{itemize}

Usually, one can distinguish between two types of epidemic dissemination of messages:\footnote{Epidemic Algorithms for Replicated Database Maintenance - A. DEMERS, M. GEALY, D. GREENE, C. HAUSER, W. IRISH, J. LARSON, S. MANNING, S. SHENKER, H. STURGIS, D. SWINEHART, D. TERRY, and D. WOODS}
\begin{itemize}
...
The peer sampling service \footnote{The Peer Sampling Service: Experimental Evaluation of Unstructured Gossip-Based Implementations - M. JELASITY, R. GUERRAOUI, A.-M. KERMARREC and M. VAN STEEN}  is a gossip-based membership dissemination protocol - it allows processes to get a subset of the whole network $V$ executing the protocol. 
...
The serivce periodically communicates with its peers to generate new samples and to adapt to churn - this creates a small network overhead.

\section{T-MAN}

T-MAN is a protocol for topology management in a gossip network. It uses a single ranking function $R$ that all nodes can apply to order any subset of potential neighbors. Processes use a profile for applying said function. The desired topology is reached when all peers are connected to their closest neigbors according to their profile and the ranking function.
With T-MAN, the current topology gradually evolves into the desired one while only using local gossip messages. 

Another important property of T-MAN is the tendency to create clusters if the ranking function does not provide some sort of anti-clustering heuristic.
...
Given the problem of multiple documents $m$ in a network of $n$ processes the peer sampling service alone will result in a high delay of message delivery and an overloaded network (see \ref{sec:Statement of the problem}) which scales poorly.

As mentioned above, T-MAN might evovle a random topology into a clustered network, depending on the chosen ranking function $R$. We will exploit this property to cluster together processes that work on the same document - now, with a structured overlay topology and interested nodes being close to each other we expect a huge improvement in message delivery speed (to relevant nodes).
This solution will work well when all documents are modified by a distinct set of nodes but as soon as certain nodes work on multiple documents we will run into the problem of how to cluster the resulting network.

Last but not least we must cover the unnecessary redundancy of messages that are send to processes which are not interested in the message.


\section{Conclusion}

We must find a ranking function $R$ that somehow reflects nodes working on multiple documents for finding an appropriate network topology. 
As we have approximate knowledge of the topology we can guess if a message is located in an area of the network that is relevant or not. A heuristic must ensure that a message does not scatter too far into the network where it is not relevant anymore.
