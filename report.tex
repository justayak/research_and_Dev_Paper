...
A gossip protocol has the advantage of being highly resilent and scalable at the same time. To broadcast a message is sends to $\mathit{c}$ (also called FANOUT) randomly selected processes from the network. Likewise, a receiving process broadcasts the message to $\mathit{c}$ randomly selected processes and so forth.

\section{Statement of the problem}

Assuming that all processes are connected to one peer-to-peer network and that a very high number of $\mathit{n}$  processes will work on a moderate to high number of $\mathit{m}$ Documents ($n \geq m$)
