...
\subtitle{Handshaking in Gossip Protocols}
...
Services like PeerCDN\cite{peercdn}, webtorrent\cite{webtorrent} or Sharefest\cite{sharefest} demonstrate how decentralized services now can be easily deployed in web browsers.
This improves the user experience drastically as users can now download a torrent as easy as watching a youtube video.
However, current solutions do not provide a scalable broadcast.
This, however, is essential for other applications such as a distributed collaborative editor or a distributed social network.
Traditionally, gossip protocols are used to implement broadcast in decentralized infrastructure.
This raise the problem of deploying gossip algorithms on WebRTC.

As mentioned, gossip-based peer-to-peer protocols have been used in a various number of large scale systems serving an array of problem settings including information dissemination \cite{Demers:1987:EAR:41840.41841}, information aggregation \cite{Jelasity:2005:GAL:1082469.1082470} and network management \cite{conf/dsom/VoulgarisS03}.
...
End user devices\footnote{We define end user devices as personal computers or consumer devices like PDA's or smart phones.} rarely own a unique public ip address.
Instead, they are members in private local area networks where they are connected to the internet through routers and firewalls.
In such infrastructure, peers can only directly address each other if they share the same local network.
Routers who connect such private networks to the internet usually have only one public address. 
Additionally, the public network address might periodically change, depending on the network settings.
%This is neccessary (a) for security reasons and (b) because of the size limitations of IPv4 which result in the fact that not every internet-enabled device can have a unique IP address.
In this rather volatile environment some sort of external service is needed to establish connections interactively\footnote{Interactive Connectivity Establishment (ICE)} and to traverse network borders.
The need for a signalling service that acquaints peers with each others makes establishing connections between peers more complex, more fragile and slower than it is in local networks.
We will call this process \emph{handshaking}.
Some technologies\footnote{such as WebRTC} aggravate this situation as they cannot share peer-to-peer connections between nodes and instead need to execute the handshake again to establish a new connections.

% 2:What is the specific problem? and background
Problems emerge when a peer-to-peer overlay network is constructed with such end user devices.
...
The constant need for handshaking makes the system less scalable as gossip protocols usually promisses.

% 3:summarize main contributions. What is the general approach taken? Why significant? MUST BE GOOD!

In this report we show that \textbf{===WE DONT KNOW YET!(SCAMP vs CYCLON vs NEWSCAST vs SELF)===}.
Furthermore, we show that the handshake mechanics can be transferred to the peer-to-peer system.
This reduces the use of the global signalling service to a minimum, as it will be used only to introduce completely new nodes to the system.
...
Chapter~\ref{chap:StateOfTheArt} studies a number of gossip protocols and introduces the basic technology. 
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \While{Termination condition not reached}
            ...
            \State
            \State // expensive with handshake
...
Suppose we want to build a massive distributed collaborative editor(DCE) \cite{nedelec:hal-00921633}\cite{MassCollab} in a web browser using WebRTC to construct a peer-to-peer network.
We want the DCE to support millions of concurrent users.
The peer-to-peer approach will scale better than a traditional server-client-architecture like Google Docs (which only allows 50 users to work on one document) or Microsoft Word Online as there will not be a bottleneck.
Nevertheless, a full graph is unreasonable as it will exhaust the peers when the number of participants is too big.
